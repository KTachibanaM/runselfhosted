...
import { disableRestartForAllServices, addServiceBuild } from '../libs/docker-compose';
...
const transformDockerCompose = (
  dockerCompose: any,
  appServiceNames: string[],
  dockerFilepath: string,
  dockerContext: string,
): any => {
  let res = disableRestartForAllServices(dockerCompose);
  appServiceNames.forEach((appServiceName) => {
    res = addServiceBuild(res, appServiceName, dockerFilepath, dockerContext);
  });
  return res;
};
...
export const provisioning = async (appId: string) => {
  ...
  try {
    ...
    let dockerCompose = yaml.safeLoad(await getGitRepoFileContent(app.gitUrl, app.nextGitHash, dockerComposePath));
    // TODO: customize appServiceNames
    dockerCompose = transformDockerCompose(dockerCompose, ['rsshub'], 'Dockerfile', '.');
    const dockerComposeContent = yaml.safeDump(dockerCompose);
    ...
    console.error(`Provisioning errored for app ${app.slug}`);
    console.error(e);

    setState(app.id, 'provision-errored');
  }
};
